...
describe('DELETE /api/plants/:id', async () => {
  ...
  const insertPlant = (dbInstance: Database): number => {
  ...
  it('deletes an existing plant for valid request', async () => {
    const plantId = insertPlant(dbInstance);
    const event = createMockH3Event({
      body: {},
      params: { id: plantId.toString() },
    });
    const response = (await handler.handler(event, dbInstance)) as { success: boolean };
