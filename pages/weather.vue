  ...
  <div>
        ...
        <v-form @submit.prevent="getWeather">
          <v-row>
            ...
            <v-col cols="12" sm="4" md="3" class="d-flex align-center">
              <v-btn color="primary" block type="submit" :loading="loading" :disabled="!city.trim()">
                Get Weather
              </v-btn>
            </v-col>
          </v-row>
    ...
    <template v-if="weather">
            ...
            <v-card-text>
              ...
              <v-row class="mt-6">
                ...
                <v-col cols="6" sm="3">
                  ...
                  <div class="text-h6">{{ (weather.visibility / 1000).toFixed(1) }} km</div>
                </v-col>
              </v-row>
            </v-card-text>
          </v-card>
          ...
          <v-card class="mb-6">
            ...
            <v-card-text>
              <v-alert color="info" variant="tonal" class="mb-4">
                ...
                Here are some plant care tips based on the current weather conditions.
              </v-alert>
              ...
              <v-list>
                ...
                <v-list-item>
                  ...
                  <v-list-item-title> Temperature Considerations </v-list-item-title>
                  <v-list-item-subtitle>
      ...
      <v-card v-if="forecast && forecast.length">
        ...
        <v-card-text>
          <v-row>
            <v-col v-for="(day, index) in forecast" :key="index" cols="12" sm="6" md="4" lg="2">
              <v-card variant="outlined" class="text-center h-100">
                ...
                <v-card-text>
                  <img :src="`https://openweathermap.org/img/w/${day.icon}.png`" :alt="day.description" height="50" />
    ...
    <div v-else-if="!loading && !weather" class="text-center pa-5">
      ...
      <p>Search for a city to see the current weather and plant care recommendations.</p>
    </div>
  </div>
</template>
...
function getLightAdvice() {
  ...
  } else if (weather.value.description.includes("rain") || weather.value.description.includes("drizzle")) {
