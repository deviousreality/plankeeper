...
<script setup>
  ...
  const upcomingTasks = computed(() => {
    ...
    if (careSchedule.value.lastWatered && careSchedule.value.wateringInterval) {
      ...
      nextWatering.setDate(
        lastWatered.getDate() + parseInt(careSchedule.value.wateringInterval)
      );
    ...
    if (
      careSchedule.value.lastFertilized &&
      careSchedule.value.fertilizingInterval
    ) {
      ...
      nextFertilizing.setDate(
        lastFertilized.getDate() +
          parseInt(careSchedule.value.fertilizingInterval)
      );
  ...
  function getTaskColor(task) {
    ...
    const diffDays = Math.ceil(
      (task.date - new Date()) / (1000 * 60 * 60 * 24)
    );
  ...
  async function saveCareSchedule() {
    ...
    try {
      const response = await fetch(
        `/api/care-schedules/${currentPlant.value.id}`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            wateringInterval:
              parseInt(careSchedule.value.wateringInterval) || null,
            fertilizingInterval:
              parseInt(careSchedule.value.fertilizingInterval) || null,
  ...
  async function logFertilizing() {
    ...
    try {
      ...
      // Update the care schedule
      careSchedule.value.lastFertilized = new Date()
        .toISOString()
        .split("T")[0];
      await saveCareSchedule();
    } catch (error) {
  ...
  async function addCareLogEntry() {
    if (
      !currentPlant.value ||
      !newLog.value.actionType ||
      !newLog.value.actionDate
    ) {
      return;
    }
