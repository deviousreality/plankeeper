    ...
    <v-card class="mt-4">
      ...
      <v-card-text>
        <div class="taxonomy-diagram">
          ...
          <v-alert
            v-if="
              !familyList.length && !geniusList.length && !speciesList.length
            "
            color="info"
          ...
          <div v-else class="taxonomy-tree">
            ...
            >
              ...
              <div v-if="family.geniusId" class="genius-node">
                ...
                >
                  <div class="taxonomy-label species">
                    ...
                    {{
                      getSpeciesNameById(
                        getGeniusByIdWithSpecies(family.geniusId)?.speciesId
                      )
                    }}
                  </div>
                </div>
              ...
              <div
                v-if="family.speciesId && !family.geniusId"
                class="species-node"
              >
                <div class="taxonomy-label species">
                  Species: {{ getSpeciesNameById(family.speciesId) }}
                </div>
              </div>
            </div>
            ...
            <div
              v-for="genius in geniusNotInFamily"
              :key="`genus-${genius.id}`"
...
<script setup>
  ...
  const geniusNotInFamily = computed(() => {
    return geniusList.value.filter(
      (genius) =>
        !familyList.value.some((family) => family.geniusId === genius.id)
    );
  });

  const speciesNotInGenusOrFamily = computed(() => {
    const speciesInGenius = geniusList.value
      .filter((g) => g.speciesId)
      .map((g) => g.speciesId);

    const speciesInFamily = familyList.value
      .filter((f) => f.speciesId)
      .map((f) => f.speciesId);

    const usedSpeciesIds = new Set([...speciesInGenius, ...speciesInFamily]);

    return speciesList.value.filter(
      (species) => !usedSpeciesIds.has(species.id)
    );
  });
  ...
  function isItemInUse(type, item) {
    ...
    switch (type) {
      case "species":
        return (
          geniusList.value.some((g) => g.speciesId === item.id) ||
          familyList.value.some((f) => f.speciesId === item.id)
        );
      case "genius":
  ...
  async function saveEdit() {
    ...
    try {
      const response = await fetch(
        `/api/taxonomy/${editingType.value}/${editingItem.value.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        if (editingType.value === "species") {
          const index = speciesList.value.findIndex(
            (s) => s.id === editingItem.value.id
          );
          if (index >= 0) speciesList.value[index] = { ...editingItem.value };
        } else if (editingType.value === "genius") {
          const index = geniusList.value.findIndex(
            (g) => g.id === editingItem.value.id
          );
          if (index >= 0) geniusList.value[index] = { ...editingItem.value };
        } else if (editingType.value === "family") {
          const index = familyList.value.findIndex(
            (f) => f.id === editingItem.value.id
          );
  ...
  async function deleteItem() {
    ...
    try {
      const response = await fetch(
        `/api/taxonomy/${deletingType.value}/${deletingItem.value.id}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (response.ok) {
        ...
        if (deletingType.value === "species") {
          speciesList.value = speciesList.value.filter(
            (s) => s.id !== deletingItem.value.id
          );
        } else if (deletingType.value === "genius") {
          geniusList.value = geniusList.value.filter(
            (g) => g.id !== deletingItem.value.id
          );
        } else if (deletingType.value === "family") {
          familyList.value = familyList.value.filter(
            (f) => f.id !== deletingItem.value.id
          );
        }
      }
    } catch (error) {
